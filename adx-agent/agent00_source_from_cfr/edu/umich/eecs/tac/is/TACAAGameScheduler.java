...
public class TACAAGameScheduler
extends HttpPage {
    ...
    public TACAAGameScheduler(InfoServer infoServer, SimServer simServer, String header) {
    ...
    private void handlePreview(HttpRequest req, StringBuffer page) {
        ...
        try {
            ...
            long time = TACAAGameScheduler.parseServerTimeDate(req.getParameter("time"));
            ...
            int[][] games = TACAAGameScheduler.scheduleGames(nrUsers, this.agentsPerGame);
            int minGames = 4 * games.length;
            int perAgent = 4 * this.gamesPerAgent(nrUsers);
    ...
    private void handleExecute(HttpRequest req, StringBuffer page) {
        ...
        try {
            ...
            if (scheduledGames == null) {
                ...
                while (i3 < n) {
                    int[] roles = new int[scheduledGames[i3].length];
                    int j = 0;
                    while (j < scheduledGames[i3].length) {
                        roles[j] = 5;
                        ++j;
                    }
                    int k = 0;
                    while (k < 4) {
                        schedule.addSimulation((int[])scheduledGames[i3], roles);
                        ++k;
                    }
                    ++i3;
                }
    ...
    private void handleConfiguration(HttpRequest req, StringBuffer page) {
        ...
        if (comps != null) {
            ...
            while (i < n) {
                ...
                page.append(comp.getID()).append("</td><td>").append(comp.getName()).append("</td><td>").append(TACAAGameScheduler.formatServerTimeDate(comp.getStartTime())).append("</td><td>").append(TACAAGameScheduler.formatServerTimeDate(comp.getEndTime())).append("</td><td>");
        ...
        if (users.length % 7 > 0) {
            page.append("<td colspan=").append(7 - users.length % 7).append(">&nbsp;</td>");
        }
    ...
    public int gamesPerAgent(int noAgents) {
        ...
        return this.agentsPerGame / TACAAGameScheduler.findLargestDivisor(noAgents, this.agentsPerGame);
    }
    ...
    public static int[][] scheduleGames(int noAgents, int agentsPerGame) {
        ...
        if (noAgents <= agentsPerGame) {
            ...
            int perAgent = agentsPerGame / TACAAGameScheduler.findLargestDivisor(noAgents, agentsPerGame);
    ...
    public static void main(String[] args) {
        ...
        int aPerGame = 8;
        int[][] games = TACAAGameScheduler.scheduleGames(gameNr, aPerGame);
