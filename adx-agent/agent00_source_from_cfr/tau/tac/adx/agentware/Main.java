...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String configFile = config.getArgument("config", "config/aw.conf");
        try {
        ...
        try {
            Class class_ = Class.forName(agentImpl);
        }
        catch (Exception e) {
            System.err.println("Could not find the agent implementation '" + agentImpl + '\'');
            e.printStackTrace();
            System.exit(1);
        }
        ...
        SimClient client = new SimClient(config, host, port, name, password, agentImpl);
    }
}
